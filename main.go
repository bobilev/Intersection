...
func naivIntersection(n,m []int) []int {
	// Работает с (не)упорядоченными множествами
	// O(n*m)
	// простой перебор, это самый не эффективный и скучный алгоритм
	// вставка нового элемента в итоговое множество производится за О(1), эта константа не влияет на общую асимптотику
	result := []int{}
	for _,val := range n {
		for _,jval := range m {
			if val == jval {
...
func sortIntersection(n,m []int) []int {
	// Работает с упорядоченными множествами
	// O(n+m)
	// так как перед нами упорядоченные множества, мы можем этим воспользоваться.
	// если элементы отсортированы по возрастанию то при проверке двух параллельных элементов двух множеств, алгоритм
	...
	// вставка нового элемента в итоговый массив производится за О(1), эта константа не влияет на общую асимптотику
	result := []int{}
	var i,j int
	for i < len(n) && j < len(m){
		if n[i] == m[j] {
...
func HashMapIntersection(n,m []int) []int {
	// Работает с (не) упорядоченными множествами
	// O(n+m)
	// поместить первое(любое) множество в хеш таблицу, и искать элементы второго множества в хеш таблице.
	// поиск в хеш таблице занимает O(1) константное время и поэтому не влияет на общую асимптотику.
	// преимущество этого алгоритма в том, что мы получаем асимптотику O(n+m) как при отсортированном множестве, даже если
	// множества не отсортированы
	result := []int{}
...
func HvangAndLinIntersection(n,m []int) []int {
	...
	// либо выполнит в них бинарный поиск для правильной точки пересечения. И если (не)найдет, то сдвинет позицию n на 1
	result,bigInt,minInt := []int{},[]int{},[]int{}
	if len(n) > len(m) {
		bigInt = n
		minInt = m
	...
	for i < len(minInt) {
		if minInt[i] == bigInt[j] {
			...
			if minInt[i] < bigInt[j] {
				...
				// Это обход ловушки, в котрую мы можем попасть из-за особенности алгоритма
				// если последний элемент n будет больше последнего элемента m
...
func MemHvangAndLinIntersection(n,m []int) []int {
	...
	// чтобы не включать их в следующие m4 элемента повторно. Так мы сможем двигаться немного быстрее.
	// В лучшем случае и среднем случае это (возможно) повлияет на производительность. Затрудняюсь вывести точную асимтотику
	result,bigInt,minInt := []int{},[]int{},[]int{}
	if len(n) > len(m) {
		bigInt = n
		minInt = m
...
func FullHvangAndLinIntersection(n,m []int) []int {
	...
	// ищу n число в отрезке из 4 элементов m множества бинарным поиском,
	// запоминаю большее из чисел, которое меньше искомого. И теперь меняем множества между собой
	...
	// O(n + (m/4)log m) - в среднем случае как будто это MemHvangAndLinIntersection или просто HvangAndLinIntersection
	// O(n+m) в худшем случае, как и остальные два алгоритма. То есть мы не потеряли а только приобрели (возможный) прирост.
	// Наверное, его надо на доске словами объяснять, мне кажется, я неявно описал его.
	result := []int{}
	...
	// i,j - увеличиваются на 4, и их предел это конец своих массивов
	for i < len(n) && j < len(m){
